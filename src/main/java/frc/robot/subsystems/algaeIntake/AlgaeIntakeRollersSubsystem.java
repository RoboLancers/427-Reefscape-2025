...
import frc.robot.Constants.AlgaeConstants;
 ...
 public class AlgaeIntakeRollersSubsystem extends SubsystemBase {
   ...
   private PIDController pidController = new PIDController(AlgaeConstants.kP, AlgaeConstants.kI, AlgaeConstants.kD);
   private SparkAbsoluteEncoder absEncoder; 
   private ArmFeedforward feedForward = new ArmFeedforward(0, 0, AlgaeConstants.kG);
   ...
   public AlgaeIntakeRollersSubsystem() {
    ...
    //  this.beambreakvalue = false;
     this.LimitSwitchTop = rollerMotor.getForwardLimitSwitch();
     this.LimitSwitchBottom = rollerMotor.getReverseLimitSwitch();
 ...
 public Command intake(){
   return run(() -> {
     rollerMotor.setVoltage(Constants.AlgaeConstants.kIntakeVoltage);
   });
 }

 public Command outtake(){
   return run(()-> {
     rollerMotor.setVoltage(Constants.AlgaeConstants.kOuttakeVoltage);
   });
 }
 ...
 public Command tune() {
   ...
   return run(() -> { 
     this.pidController = new PIDController(SmartDashboard.getNumber("/AlgaeIntakeRollers/kP", 0), SmartDashboard.getNumber("/AlgaeIntakeRollers/kI", 0), SmartDashboard.getNumber("/AlgaeIntakeRollers/kG", 0));
     this.feedForward = new ArmFeedforward(0, SmartDashboard.getNumber("/AlgaeIntakeRollers/kG", 0), 0);
     goToAngle(SmartDashboard.getNumber("AlgaeIntakeRollers/desiredAngle", 0));
   });
 }
}